...
export default function NoteItem({ note: initialNote }: Props) {
  ...
  const [note, setNote] = useState(initialNote); // Add this line
  ...
  return (
    <div
      key={note._id}
      className="bg-green-950 p-2 my-2 flex justify-between hover:bg-zinc-900 hover:cursor-pointer gap-x-2"
    >
      <div>
        <input
          name="title"
      ...
      <div className="flex gap-x-2">
        <button onClick={() => updateNote(note._id, { active: !note.active })}>
          {note.active ? (
            ...
            <IoCheckmarkCircle className="text-gray-400" />
          )}
        </button>
        <button
          onClick={() => {
